...
def get_person(person_id: int):
    with get_conn() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Employees WHERE ID = ?", person_id)
...
def get_conn():
    ...
    conn = pyodbc.connect(os.getenv("AZURE_SQL_CONNECTIONSTRING"))
    return conn
...
def run_conversation(user_input):
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                "name": "get_person",
                "description": "Get the employee with a given ID",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "person_id": {"type": "string", "description": "The ID of an employee"},
                    },
                    "required": ["person_id"],
                },
            },
